...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select("id,token,label,used_at,expires_at,created_at,contract_id,discount_percent,promo_percent")
...
export async function POST(req: Request) {
  ...
  const promo_percent = Math.max(0, Math.min(50, Number(body.promo_percent) || 0));
  ...
  const { error } = await supabaseAdmin
    .from("contract_tokens")
    .insert([{ token, label, expires_at, discount_percent, promo_percent }]);
